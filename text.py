...
def check_answer():
    ...
    print(f"Number of Question: {main_win.total}")
    print(f"Score: {main_win.score/main_win.total * 100:.2f}")
...
def next_question():
    question_index = randint(0, len(questions_list) - 1)
...
def click_ok():
    if btn.text() == 'Answer' and any([r.isChecked() for r in btn_list]):
        check_answer()
    if btn.text() == "Next question":
        next_question()


# ask("What is Python?", "programming language", "Snake", "Animal", "Birt")
# q = Question(
#     question='Who is your python teacher?', 
#     cor='T.Natt', 
#     w1='T.Apireak', w2='T.Sophal', w3='T.Thida')
# ask(q)
